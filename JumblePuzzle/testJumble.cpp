...
void testJumble()
{
    ...
    /*** Testing BadJumblePuzzle Exceptions ***/
    cout << "\nTesting BadJumblePuzzle Exceptions" << endl;
    // Testing Size less than 3 characters
    try
    {
        JumblePuzzle jp1("NO", "easy");
        cout << "Testing Size (<3): Unsuccessful" << endl;
    }
    catch (BadJumbleException)
    {
        cout << "Testing Size (<3): Successful" << endl;
    }

    // Testing Size greater than 10 characters
    try
    {
        JumblePuzzle jp1("THISISPLUSTEN", "easy");
        cout << "Testing Size (>10): Unsuccessful" << endl;
    }
    catch (BadJumbleException)
    {
        cout << "Testing Size (>10): Successful" << endl;
    }

    // Testing Difficulty
    try
    {
        JumblePuzzle jp1("HELLO", "friedchicken");
        cout << "Testing Difficulty: Unsuccessful" << endl;
    }
    catch (BadJumbleException)
    {
        cout << "Testing Difficulty: Successful" << endl;
    }

    cout << "\nTesting Getters" << endl;
    // Testing Getters
    JumblePuzzle testPuzzle;
    string hiddenWord = "HELLO";
    testPuzzle = JumblePuzzle(hiddenWord, "medium");

    int size = testPuzzle.getSize();
    int row = testPuzzle.getRowPos();
    int col = testPuzzle.getColPos();
    int actualSize = hiddenWord.size() * 3;
    char direction = testPuzzle.getDirection();
    charArrayPtr* jumble = testPuzzle.getJumble();

    // Test Size Get
    if (actualSize != size)
        cout << "Testing Size: Unsuccessful" << endl;
    else
        cout << "Testing Size: Successful" << endl;

    // Test Row Get
    if (row < 0 || row > hiddenWord.size()* 4)
        cout << "Testing Row: Unsuccessful" << endl;
    else
        cout << "Testing Row: Successful" << endl;

    // Test Column Get
    if (col < 0 || col > hiddenWord.size()* 3)
        cout << "Testing Col: Unsuccessful" << endl;
    else
        cout << "Testing Col: Successful" << endl;

    // Test Direction Get
    if (direction != 'n' &&  direction != 'e' && direction != 's' && direction != 'w')
        cout << "Testing Direction: Unsuccessful" << endl;
    else
        cout << "Testing Direction: Successful" << endl;

    //Test Jumble Get
    /*if (jumble[row][col] != hiddenWord.at(0))
        cout << "Testing Jumble: Unsuccessful" << endl;
    else
        cout << "Testing Jumble: Successful" << endl;
*/
